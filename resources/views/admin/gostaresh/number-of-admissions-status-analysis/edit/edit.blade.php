...
@section('page-title')
    ...
    <span>
        <a href="{{ route('number.of.admissions.status.analysis.index') }}" class="btn btn-secondary btn-sm">بازگشت به لیست اطلاعات</a>
    </span>
@endsection
            ...
            <div class="card">
                <div class="card-body" id="app">
                    <form class="form-horizontal" method="POST"
                            ...
                            <div class="col-sm-10">
                                <select name="grade_id" id="grade_id" class="form-select">
                                    ...
                                    @foreach (\App\Models\Grade::all() as $grade)
                                        <option {{ $grade->id == $numberOfAdmissionsStatusAnalysis->grade_id ? ' selected' : '' }} value="{{ $grade->id}}">{{ $grade->name }}</option>
                                    @endforeach
                                </select>
                        ...
                        <x-select-year :default="$numberOfAdmissionsStatusAnalysis->year" min="{{ config('gostaresh.year.min', 1370) }}" max="{{ config('gostaresh.year.max', 1405) }}" :required="false" name="year"></x-select-year>
