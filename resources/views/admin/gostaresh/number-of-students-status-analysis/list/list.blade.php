...
@section('page-title')
    ...
    @can("create-any-NumberOfStudentsStatusAnalysis")
        <span>
...
@section('content')
                            ...
                            @foreach ($numberOfStudentsStatusAnalysises as $key => $numberOfStudentsStatusAnalysis)
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        @can("edit-any-NumberOfStudentsStatusAnalysis")
                                            <a href="{{ route('number.of.students.status.analysis.edit', $numberOfStudentsStatusAnalysis) }}"
                                               title="{{ __('validation.buttons.edit') }}"
                                               class="btn btn-warning btn-sm"><i class="fa fa-edit"></i></a>
                                        @endcan

                                        @can("delete-any-NumberOfStudentsStatusAnalysis")
                                            <form method="POST"
                                                  action="{{ route('number.of.students.status.analysis.destroy', $numberOfStudentsStatusAnalysis) }}"
                                                  role="form">
                                                @csrf
                                                {{ method_field('delete') }}
    ...
    <div class="row">
        <div class="col-md-12">
            <x-gostaresh.number-of-students-status-analysis.line-chart-all-fields-by-year-component/>
        </div>
    </div>
@endsection
