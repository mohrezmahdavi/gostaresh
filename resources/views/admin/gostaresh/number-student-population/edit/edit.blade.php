            ...
            <div class="card">
                <div class="card-body" id="app">
                    <form class="form-horizontal" method="POST"
                        action="{{ route('number.student.population.update', $numberStudentPopulation) }}" role="form">
                        @csrf
                        @method('PUT')

                        <select-province-component province_default="{{ $numberStudentPopulation->province_id ?? ''}}"
                            ...
                            <div class="col-sm-10">
                                <select name="gender_id" id="gender_id" class="form-select">
                                    @foreach (config('gostaresh.gender') as $key => $value)
                                        <option {{ $key == $numberStudentPopulation->gender_id ? 'selected' : '' }}
                        ...
                        <x-select-year :default="$numberStudentPopulation->year" min="{{ config('gostaresh.year.min', 1370) }}" max="{{ config('gostaresh.year.max', 1405) }}" :required="false" name="year"></x-select-year>
