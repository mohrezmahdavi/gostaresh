...
@section('page-title')
    ...
    <span>
        <a href="{{ route('status.analysis.of.the.number.of.fields.of.study.index') }}" class="btn btn-secondary btn-sm">بازگشت به لیست اطلاعات</a>
    </span>
@endsection
            ...
            <div class="card">
                <div class="card-body" id="app">
                    <form class="form-horizontal" method="POST"
                        action="{{ route('status.analysis.of.the.number.of.fields.of.study.update', $stsAnlysOfTheNumOfFieldsOfStudy) }}"
                        role="form">
                        @csrf
                        @method('PUT')

                        <select-province-component province_default="{{ $stsAnlysOfTheNumOfFieldsOfStudy->province_id }}"
                            zone_default="{{ $stsAnlysOfTheNumOfFieldsOfStudy->county->zone }}"
                            county_default="{{ $stsAnlysOfTheNumOfFieldsOfStudy->county_id }}"
                            city_default="{{ $stsAnlysOfTheNumOfFieldsOfStudy->city_id }}"
                        ...
                        <x-select-year :default="$stsAnlysOfTheNumOfFieldsOfStudy->year" min="{{ config('gostaresh.year.min', 1370) }}" max="{{ config('gostaresh.year.max', 1405) }}" :required="false" name="year"></x-select-year>
