<div class="card">
    <div class="card-body">
        ...
        <div class="d-flex align-items-start justify-content-between">
            <div>
                <h4 class="header-title">درصد وضعیت هنر و سرگرمی</h4>
            </div>
            <div class="avatar-sm">
        ...
        <div class="mt-3 text-center" >
            @if (count($employmentOfProvincials) > 0)
                <div id="projections-actuals16" class="apex-charts"></div>
            @else
...
if (count($employmentOfProvincials) > 0) {
    ...
    foreach (config('gostaresh.employment_status') as $key => $value) {
        $number = $employmentOfProvincials->where('art_and_entertainment', $key)->first()?->total;
...
@if (count($employmentOfProvincials) > 0)
    <script>
        ...
        var chart = new ApexCharts(document.querySelector("#projections-actuals16"), options);
